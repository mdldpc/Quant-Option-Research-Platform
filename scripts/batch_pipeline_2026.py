...
OUTPUT_ROOT = Path(r"D:\Quant_Option_Project\data_parquet\batch_2026")
...
def process_one_file(file_path):
    ...
    if trade_date.weekday() >= 5:
        print(f"SKIP {trade_date_str}: weekend file")
        return

    out_dir = OUTPUT_ROOT / f"trade_date={trade_date_str}"

    success_flag = out_dir / "SUCCESS.flag"
    final_file = out_dir / "greeks_spline.parquet"

    if success_flag.exists() and final_file.exists():
        print(f"SKIP {trade_date_str}: already completed")
        return
    ...
    # 如果之前中断过，清理半成品，避免混淆
    for temp_file in [
        "option_table.parquet",
        "iv_table.parquet",
        "iv_spline.parquet",
        "greeks_spline.parquet",
        "SUCCESS.flag",
    ]:
        path = out_dir / temp_file
        if path.exists():
            path.unlink()
    ...
    print("DONE:", trade_date_str)


def main():
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

    files = sorted(RAW_ROOT.glob("*.csv.xz"))

    print("Total 2026 files:", len(files))
    ...
    for file_path in files:
        try:
            ...
            print("\nInterrupted by user.")
            print("You can rerun the script later; completed dates will be skipped.")
            raise
